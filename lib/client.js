...
function MQTTClient(adapter) {
    ...
    function updateStates(snipsID,slot,value) {       
        ...
        let snipsName;
        let snipsDesc;
        let snipsType = 'string';
        let snipsRole = 'text';
        
        switch (slot) {
            case 'continueSession':
                snipsPath = 'send.continueSession';
            case 'text':
            ...
            break;
            case 'intentName':
                snipsPath = 'intentName';
                snipsName = 'name of the recognized intent';
                snipsDesc = 'intent from snips device';
            break;
            case 'compiledText':
            ...
            break;
            case 'slotFolder':
                snipsPath = 'receive.slotFolder';
                snipsName = 'received compiled folder';
                snipsDesc = "receive recognized folder from snip's intents of snips device";
            break;
            case 'slotTime':
            ...
            break;
            case 'slotExpletive':
                snipsPath = 'receive.slotExpletive';
                snipsName = 'received compiled expletive';
                snipsDesc = "receive recognized expletive from snip's intents of snips device";
            break;
            case 'slotConfirmation':
                snipsPath = 'receive.slotConfirmation';
                snipsName = 'received compiled confirmation';
                snipsDesc = "receive recognized confirmation from snip's intents of snips device";
            break;
            case 'sessionStarted':
            case 'sessionEnded':
                snipsPath = 'sessionID';
        ...
        adapter.setObjectNotExists(
            adapter.namespace + '.devices.' + snipsID + '.send.continueSession', {
                type: 'state',
                common: {
                    name: 'continue session with output',
                    desc: 'session from script',
                    type: 'string',
                    role: 'text',
                    read: true,
                    write: true
        ...
        adapter.setObjectNotExists(
            adapter.namespace + '.devices.' + snipsID + '.send.response', {
                type: 'state',
                common: {
                    name: 'response for output',
                    desc: 'response from script',
                    type: 'string',
                    role: 'text',
                    read: true,
                    write: true
    ...
    function send2Server(id, state, cn) {
        ...
        switch (cn) {
            ...
            break;
            case 'inject_folder':
                client.publish(id, JSON.stringify({"operations":[["add",new function(){ this[adapter.config.snipsLanguage + ".iobroker.folder"] = [state]; }]]}), {qos: adapter.config.defaultQoS, retain: adapter.config.retain});
            break;
            case 'feedback':
    ...
    (function _constructor(config) {
        ...
        client.on('message', (topic, message) => {
            ...
            let slotTime;
            let slotExpletive;
            let slotConfirmation;
            let result = JSON.parse(message);
            roomid = result.siteId;
            let output = result.input;
            
            let intentName;
            if (topic.search("intent")>-1) {
                //hermes/intent/kraftcome:confirmRequestDE
                intentName = topic.substring(24,topic.length);
            }
            ...
            let loopDeviceComplete = function () {
                ...
                updateStates(roomid,'intentName',intentName)
            ...
            switch (topic) {
                ...
                case 'hermes/dialogueManager/sessionEnded' :
                    ...
                    updateStates(roomid, 'slotFolder', '')
                    updateStates(roomid, 'slotTime', '')
                    updateStates(roomid, 'slotExpletive', '')
                    updateStates(roomid, 'slotConfirmation', '')
                    updateStates(roomid, 'intentName','')
                    updateStates(roomid, 'continueSession','')
                break;

                case 'hermes/intent/kraftcome:setDevice' + adapter.config.snipsLanguage.toUpperCase():
                ...
                case 'hermes/intent/kraftcome:getStatus' + adapter.config.snipsLanguage.toUpperCase():
                ...
                case 'hermes/intent/kraftcome:getFolder' + adapter.config.snipsLanguage.toUpperCase():
                    let arrFolderSlots = [];
                    let lenFolderSlots;
                    for (let i in result.slots) {
                        switch (result.slots[i].slotName) {
                            case 'device':
                                lenFolderSlots = arrFolderSlots.unshift(result.slots[i])
                            break;
                        default:
                            lenFolderSlots = arrFolderSlots.push(result.slots[i])
                        }
                    };
                    
                    for (let i in arrFolderSlots) {
                        switch (arrFolderSlots[i].slotName) {
                            case 'room':
                                adapter.getState('devices.' + roomid + '.enforceSameRoom', function (err, state) {
                                    if(state.val) {
                                        adapter.getEnums('rooms', (err, res) => {
                                            if (res) {
                                                ...
                                                output = output.replace(arrFolderSlots[i].rawValue, slotRoom)
                                                if(--statusTasksToGo === 0) {
                                                    loopStatusComplete();
                                                }
                                            } else if (err) {
                                        ...
                                        if(!slotRoom || slotRoom == '') {
                                            slotRoom = arrFolderSlots[i].value.value;
                                        } else {
                                            slotRoom = slotRoom + ", " + arrFolderSlots[i].value.value;
                                        }
                                        output = output.replace(arrFolderSlots[i].rawValue, slotRoom)
                                        if(--statusTasksToGo === 0) {
                                            loopStatusComplete();
                            ...
                            break;
                            case 'enumeration':
                                slotFolder = arrFolderSlots[i].value.value;
                                output = output.replace(arrFolderSlots[i].rawValue, slotFolder)
                            break;
                        }
                    }
                
                    updateStates(roomid,'slotFolder', slotFolder)
                    updateStates(roomid,'compiledText',output)
                    updateStates(roomid,'text',result.input)
                    updateStates(roomid,'intentName',intentName)
                ...
                case 'hermes/intent/kraftcome:SmallTalk' + adapter.config.snipsLanguage.toUpperCase():
                    let arrSmalltalkSlots = [];
                    let lenSmalltalkSlots;
                    for (let i in result.slots) {
                        switch (result.slots[i].slotName) {
                            case 'device':
                                lenSmalltalkSlots = arrSmalltalkSlots.unshift(result.slots[i])
                            break;
                        default:
                            lenSmalltalkSlots = arrSmalltalkSlots.push(result.slots[i])
                        }
                    };
                    
                    for (let i in arrSmalltalkSlots) {
                        switch (arrSmalltalkSlots[i].slotName) {
                            case 'room':
                                adapter.getState('devices.' + roomid + '.enforceSameRoom', function (err, state) {
                                    if(state.val) {
                                        adapter.getEnums('rooms', (err, res) => {
                                            if (res) {
                                                ...
                                                output = output.replace(arrSmalltalkSlots[i].rawValue, slotRoom)
                                                if(--statusTasksToGo === 0) {
                                                    loopStatusComplete();
                                                }
                                            } else if (err) {
                                        ...
                                        if(!slotRoom || slotRoom == '') {
                                            slotRoom = arrSmalltalkSlots[i].value.value;
                                        } else {
                                            slotRoom = slotRoom + ", " + arrSmalltalkSlots[i].value.value;
                                        }
                                        output = output.replace(arrSmalltalkSlots[i].rawValue, slotRoom)
                                        if(--statusTasksToGo === 0) {
                                            loopStatusComplete();
                            ...
                            break;
                            case 'expletive':
                                slotExpletive = arrSmalltalkSlots[i].value.value;
                                output = output.replace(arrSmalltalkSlots[i].rawValue, slotExpletive)
                            break;
                        }
                    }

                    updateStates(roomid,'slotExpletive', slotExpletive)
                    updateStates(roomid,'compiledText',output)
                    updateStates(roomid,'text',result.input)
                    updateStates(roomid,'intentName',intentName)
                    //adapter.setForeignState('text2command.' + config.topic + '.text', message + ' [' + result.sessionId + ']');
                ...
                case 'hermes/intent/kraftcome:requestGenre' + adapter.config.snipsLanguage.toUpperCase():
                case 'hermes/intent/kraftcome:confirmRequest' + adapter.config.snipsLanguage.toUpperCase():
                    let arrRequestSlots = [];
                    let lenRequestSlots;
                    for (let i in result.slots) {
                        switch (result.slots[i].slotName) {
                            case 'device':
                                lenRequestSlots = arrRequestSlots.unshift(result.slots[i])
                            break;
                        default:
                            lenRequestSlots = arrRequestSlots.push(result.slots[i])
                        }
                    };
                    
                    for (let i in arrRequestSlots) {
                        switch (arrRequestSlots[i].slotName) {
                            case 'room':
                                adapter.getState('devices.' + roomid + '.enforceSameRoom', function (err, state) {
                                    if(state.val) {
                                        adapter.getEnums('rooms', (err, res) => {
                                            if (res) {
                                                ...
                                                output = output.replace(arrRequestSlots[i].rawValue, slotRoom)
                                                if(--statusTasksToGo === 0) {
                                                    loopStatusComplete();
                                                }
                                            } else if (err) {
                                        ...
                                        if(!slotRoom || slotRoom == '') {
                                            slotRoom = arrRequestSlots[i].value.value;
                                        } else {
                                            slotRoom = slotRoom + ", " + arrRequestSlots[i].value.value;
                                        }
                                        output = output.replace(arrRequestSlots[i].rawValue, slotRoom)
                                        if(--statusTasksToGo === 0) {
                                            loopStatusComplete();
                            ...
                            case 'genre':
                            case 'confirmation':
                                slotConfirmation = arrRequestSlots[i].value.value;
                                output = output.replace(arrRequestSlots[i].rawValue, slotConfirmation)
                            break;
                        }
                    }                    
                    updateStates(roomid,'slotConfirmation', slotConfirmation)
                    updateStates(roomid,'compiledText',output)
                    updateStates(roomid,'text',result.input)
                    updateStates(roomid,'intentName',intentName)
                    //adapter.setForeignState('text2command.' + config.topic + '.text', message + ' [' + result.sessionId + ']');
